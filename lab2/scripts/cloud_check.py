...
def main() -> int:
    ...
    for slot in config.CAPABILITY_SLOTS:
        ...
        # Capability checks prove presence, not disclose identifiers. Some providers
        # permit credentials in URIs, so never echo configured values to terminals,
        # screenshots, CI logs, or course channels.
        results.append(line(slot, bool(value), "configured" if value else "unset"))
